...
(async function() {
    ...
    if (!modelBtn) return JSON.stringify({status: 'NO_MODEL_BTN'});
    ...
    // Search target item, e.g. "Sonnet"
    var allEls = Array.from(document.querySelectorAll('div, button, span, li, a'));
    var match = allEls.find(function(el) {
        if (el.offsetParent === null) return false;
        var t = (el.innerText || el.textContent || '').trim();
        return (t.includes('Sonnet') || t.includes('Claude Sonnet')) && el !== modelBtn && !modelBtn.contains(el);
    });
    
    if (!match) {
        document.body.click();
        return JSON.stringify({status: 'NO_MATCH_SONNET', current: beforeModel});
    }
    
    // Click target element with MouseEvent
    var clickedText = match.innerText.trim();
    ...
    // Check model button text after click
    var afterModel = modelBtn.innerText.trim();
    
    return JSON.stringify({
        status: 'OK',
        beforeModel: beforeModel,
        clickedTarget: clickedText,
...
if res and 'result' in res and 'result' in res['result']:
    print("DEBUG_MODEL_CLICK:", res['result']['result'].get('value'))
