...
(async function() {
    ...
    // Find all leaf elements (no child elements) that have text
    var all = Array.from(document.querySelectorAll('*'));
    var candidates = [];
    for (var el of all) {
        if (el.children.length === 0) {
            var txt = (el.textContent || '').trim();
            if (txt.includes('Flash') || txt.includes('Pro') || txt.includes('Sonnet') || txt.includes('Opus') || txt.includes('120B')) {
                candidates.push({
                    tag: el.tagName,
                    text: txt,
...
if res and 'result' in res and 'result' in res['result']:
    print("LEAF_MODEL_ELEMENTS:", res['result']['result'].get('value'))
