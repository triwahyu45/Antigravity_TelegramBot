...
js_test = """
(async function() {
    // Find the model button (e.g. Gemini 3.6 Flash (Medium))
    ...
    // Scan popover menu items
    var popovers = document.querySelectorAll('[role="menu"], [class*="popover"], [class*="dropdown"], div.z-50');
    var items = [];
    for (var p of popovers) {
        var els = p.querySelectorAll('button, div[role="menuitem"], div.cursor-pointer');
        for (var el of els) {
            var txt = (el.innerText || '').trim().replace(/\\n/g, ' ');
            if (txt.length > 2) items.push(txt);
        }
    }
