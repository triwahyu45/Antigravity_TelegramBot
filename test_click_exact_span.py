...
def switch_model_span(target_kw):
    escaped_kw = json.dumps(target_kw)
    js = f"""
    (async function() {{
        // 1. Find model trigger button
        ...
        // Find exact span.text-xs.text-left
        var kw = {escaped_kw}.toLowerCase();
        var spans = Array.from(document.querySelectorAll('span.text-xs.text-left, button span, div.cursor-pointer span'));
        var matchSpan = spans.find(function(s) {{
        ...
        if (!matchSpan) {{
            document.body.click();
            return 'SPAN_NOT_FOUND:' + kw + ' (Before: ' + beforeModel + ')';
        }}
        
        // Click matchSpan or its closest clickable container
        var clickTarget = matchSpan.closest('button, div[role="menuitem"], div.cursor-pointer') || matchSpan.parentElement || matchSpan;
        var targetText = matchSpan.innerText.trim();
        
        clickTarget.click();
        await new Promise(r => setTimeout(r, 300));
        
        var afterModel = modelBtn.innerText.trim();
        return 'OK:' + afterModel + ' (Target: ' + targetText + ')';
    }})()
    """
...
if __name__ == "__main__":
    kw = sys.argv[1] if len(sys.argv) > 1 else "Sonnet"
    print(f"Testing switch to '{kw}' using span selector...")
    res = switch_model_span(kw)
    print("Result:", res)
