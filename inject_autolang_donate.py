...
print(f"Mulai memperbarui Geo-IP & Auto-Language ke {len(repos)} repositori web...\n")
...
for repo_name in repos:
    ...
    # 1. Update donate-widget.js dengan fitur Geo-IP & Multi-Language
    js_path = os.path.join(repo_path, "donate-widget.js")
    with open(js_path, "w", encoding="utf-8") as f:
        f.write(widget_js_content.strip())
    print(" -> Tulis ulang donate-widget.js dengan Geo-IP & i18n support")
    ...
    subprocess.run(f'git -C "{repo_path}" commit -m "feat: add automatic Geo-IP & browser language detection (ID/EN) with manual toggle"', shell=True, capture_output=True, text=True)
...
print("=" * 60)
print(f"Selesai! Total {len(success_repos)}/{len(repos)} repositori web berhasil diperbarui dengan Auto-Language & Geo-IP Detection.")
