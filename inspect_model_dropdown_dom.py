...
js_test = """
(function() {
    var buttons = Array.from(document.querySelectorAll('button, div[role="button"], [class*="select"]'));
    var modelElements = [];
    for (var btn of buttons) {
        var txt = (btn.innerText || btn.textContent || '').trim();
        if (txt.includes('Gemini') || txt.includes('Claude') || txt.includes('GPT') || txt.includes('Flash') || txt.includes('Pro') || txt.includes('Model')) {
            modelElements.push({
                tag: btn.tagName,
    ...
    return JSON.stringify(modelElements);
})()
"""

res = cdp_send("Runtime.evaluate", {"expression": js_test, "returnByValue": True})
print("Model DOM elements found:", res)
