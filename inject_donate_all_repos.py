import os
import glob
import subprocess

base_dir = r"G:\Github TriWahyu45"

widget_js_content = """(function() {
    if (document.getElementById('tw45-donate-widget-container')) return;

    const style = document.createElement('style');
    style.innerHTML = `
        .tw45-donate-btn {
            position: fixed;
            bottom: 20px;
            right: 20px;
            z-index: 999999;
            background: linear-gradient(135deg, #ff416c, #ff4b2b);
            color: #fff;
            border: none;
            padding: 10px 18px;
            border-radius: 30px;
            font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
            font-size: 14px;
            font-weight: 600;
            box-shadow: 0 8px 24px rgba(255, 65, 108, 0.4);
            cursor: pointer;
            display: flex;
            align-items: center;
            gap: 8px;
            transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
            user-select: none;
        }
        .tw45-donate-btn:hover {
            transform: translateY(-3px) scale(1.05);
            box-shadow: 0 12px 30px rgba(255, 65, 108, 0.6);
        }
        .tw45-donate-btn:active {
            transform: translateY(0) scale(0.98);
        }
        .tw45-donate-overlay {
            position: fixed;
            top: 0; left: 0; width: 100vw; height: 100vh;
            background: rgba(0, 0, 0, 0.6);
            backdrop-filter: blur(6px);
            z-index: 999998;
            opacity: 0;
            visibility: hidden;
            transition: opacity 0.3s ease, visibility 0.3s ease;
        }
        .tw45-donate-overlay.active {
            opacity: 1;
            visibility: visible;
        }
        .tw45-donate-modal {
            position: fixed;
            bottom: 80px;
            right: 20px;
            width: 320px;
            max-width: calc(100vw - 40px);
            background: #18191c;
            border: 1px solid rgba(255, 255, 255, 0.12);
            border-radius: 20px;
            padding: 22px;
            box-shadow: 0 20px 40px rgba(0, 0, 0, 0.7);
            z-index: 999999;
            font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
            color: #fff;
            opacity: 0;
            visibility: hidden;
            transform: translateY(20px) scale(0.95);
            transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
        }
        .tw45-donate-modal.active {
            opacity: 1;
            visibility: visible;
            transform: translateY(0) scale(1);
        }
        .tw45-modal-header {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-bottom: 14px;
        }
        .tw45-modal-title {
            font-size: 16px;
            font-weight: 700;
            display: flex;
            align-items: center;
            gap: 6px;
            color: #fff;
        }
        .tw45-close-btn {
            background: rgba(255, 255, 255, 0.1);
            border: none;
            color: #aaa;
            width: 28px;
            height: 28px;
            border-radius: 50%;
            cursor: pointer;
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 14px;
            transition: background 0.2s;
        }
        .tw45-close-btn:hover {
            background: rgba(255, 255, 255, 0.2);
            color: #fff;
        }
        .tw45-modal-desc {
            font-size: 12px;
            color: #88888e;
            margin-bottom: 16px;
            line-height: 1.4;
        }
        .tw45-donate-links {
            display: flex;
            flex-direction: column;
            gap: 10px;
        }
        .tw45-donate-link {
            display: flex;
            align-items: center;
            justify-content: space-between;
            padding: 12px 16px;
            border-radius: 12px;
            text-decoration: none;
            color: #fff;
            font-size: 13px;
            font-weight: 600;
            transition: transform 0.2s, box-shadow 0.2s;
        }
        .tw45-donate-link:hover {
            transform: translateX(4px);
        }
        .tw45-link-saweria {
            background: linear-gradient(135deg, #fa709a 0%, #fee140 100%);
            color: #111;
        }
        .tw45-link-trakteer {
            background: linear-gradient(135deg, #e11d48 0%, #be123c 100%);
            color: #fff;
        }
        .tw45-link-linktree {
            background: linear-gradient(135deg, #10b981 0%, #059669 100%);
            color: #fff;
        }
        .tw45-footer-tag {
            margin-top: 14px;
            text-align: center;
            font-size: 11px;
            color: #555;
        }
    `;
    document.head.appendChild(style);

    const container = document.createElement('div');
    container.id = 'tw45-donate-widget-container';
    container.innerHTML = `
        <div class="tw45-donate-overlay" id="tw45-overlay"></div>
        <button class="tw45-donate-btn" id="tw45-btn">
            ❤️ <span>Dukung / Donate</span>
        </button>
        <div class="tw45-donate-modal" id="tw45-modal">
            <div class="tw45-modal-header">
                <div class="tw45-modal-title">💖 Dukung Karya Ini</div>
                <button class="tw45-close-btn" id="tw45-close">✕</button>
            </div>
            <div class="tw45-modal-desc">
                Terima kasih telah menggunakan aplikasi ini! Dukungan kamu sangat berarti untuk pengembangan proyek ini.
            </div>
            <div class="tw45-donate-links">
                <a href="https://saweria.co/triwahyu45" target="_blank" rel="noopener noreferrer" class="tw45-donate-link tw45-link-saweria">
                    <span>💛 Saweria (QRIS / DANA / OVO)</span> ➔
                </a>
                <a href="https://trakteer.id/triwahyu45" target="_blank" rel="noopener noreferrer" class="tw45-donate-link tw45-link-trakteer">
                    <span>☕ Trakteer (Kopi & Dukungan)</span> ➔
                </a>
                <a href="https://linktr.ee/triwahyu45" target="_blank" rel="noopener noreferrer" class="tw45-donate-link tw45-link-linktree">
                    <span>🌳 Linktree Tri Wahyu (Semua Link)</span> ➔
                </a>
            </div>
            <div class="tw45-footer-tag">Dibuat oleh Tri Wahyu (@triwahyu45)</div>
        </div>
    `;
    document.body.appendChild(container);

    const btn = document.getElementById('tw45-btn');
    const modal = document.getElementById('tw45-modal');
    const overlay = document.getElementById('tw45-overlay');
    const closeBtn = document.getElementById('tw45-close');

    function toggleModal(show) {
        if (show) {
            modal.classList.add('active');
            overlay.classList.add('active');
        } else {
            modal.classList.remove('active');
            overlay.classList.remove('active');
        }
    }

    btn.addEventListener('click', () => toggleModal(!modal.classList.contains('active')));
    closeBtn.addEventListener('click', () => toggleModal(false));
    overlay.addEventListener('click', () => toggleModal(false));
})();
"""

repos = [d for d in os.listdir(base_dir) if os.path.isdir(os.path.join(base_dir, d)) and not d.startswith('.')]

print(f"Mulai menyuntikkan Donate Widget ke {len(repos)} repositori web...\n")

success_repos = []

for repo_name in repos:
    repo_path = os.path.join(base_dir, repo_name)
    print("=" * 60)
    print(f"Memproses repositori: [{repo_name}]")
    
    # 1. Tulis donate-widget.js
    js_path = os.path.join(repo_path, "donate-widget.js")
    with open(js_path, "w", encoding="utf-8") as f:
        f.write(widget_js_content.strip())
    print(" -> Tulis file donate-widget.js")
    
    # 2. Injeksi <script src="donate-widget.js"></script> ke semua berkas HTML
    html_files = glob.glob(os.path.join(repo_path, "**", "*.html"), recursive=True)
    injected_count = 0
    
    for hfile in html_files:
        with open(hfile, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()
            
        if "donate-widget.js" not in content:
            script_tag = '<script src="donate-widget.js"></script>\n'
            if "</body>" in content:
                content = content.replace("</body>", f"{script_tag}</body>")
            else:
                content += f"\n{script_tag}"
                
            with open(hfile, "w", encoding="utf-8") as f:
                f.write(content)
            injected_count += 1
            print(f" -> Injeksi <script> ke {os.path.basename(hfile)}")
            
    # 3. Commit dan push ke GitHub
    print(f" -> Commit & Push ke GitHub...")
    subprocess.run(f'git -C "{repo_path}" config user.name "triwahyu45"', shell=True)
    subprocess.run(f'git -C "{repo_path}" config user.email "triwahyu45@gmail.com"', shell=True)
    subprocess.run(f'git -C "{repo_path}" add .', shell=True)
    commit_res = subprocess.run(f'git -C "{repo_path}" commit -m "feat: add universal floating donate widget (Saweria, Trakteer, Linktree)"', shell=True, capture_output=True, text=True)
    
    push_res = subprocess.run(f'git -C "{repo_path}" push origin main', shell=True, capture_output=True, text=True)
    if push_res.returncode == 0:
        print(f"[OK] Sukses Push: {repo_name}")
        success_repos.append(repo_name)
    else:
        # Coba master branch jika main gagal
        push_res_master = subprocess.run(f'git -C "{repo_path}" push origin master', shell=True, capture_output=True, text=True)
        if push_res_master.returncode == 0:
            print(f"[OK] Sukses Push (master): {repo_name}")
            success_repos.append(repo_name)
        else:
            print(f"[WARN] Push failed: {push_res.stderr.strip() or push_res_master.stderr.strip()}")

print("=" * 60)
print(f"🎉 Selesai! Total {len(success_repos)}/{len(repos)} repositori web berhasil diperbarui dengan Donate Widget!")
