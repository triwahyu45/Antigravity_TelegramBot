...
js_test = """
(function() {
    var imgs = Array.from(document.querySelectorAll('img')).map(i => i.src).filter(Boolean);
    return JSON.stringify(imgs.slice(-10));
})()
"""

res = cdp_send("Runtime.evaluate", {"expression": js_test, "returnByValue": True})
if res and 'result' in res and 'result' in res['result']:
    print("Images found in DOM:", res['result']['result'].get('value'))
