...
(async function() {
    ...
    // Insert text
    document.execCommand('insertText', false, 'Tes async submit button');
    input.dispatchEvent(new Event('input', { bubbles: true }));

    // Wait 200ms for React re-render
    await new Promise(r => setTimeout(r, 200));

    // Find send button after React re-render
    ...
    if (btn) {
        btn.click();
        return 'SUCCESS_AFTER_WAIT: clicked ' + btn.className;
    } else {
        // Backup: press Enter key
        input.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true}));
        return 'ENTER_KEY_FALLBACK';
    }
})()
"""

res = cdp_send("Runtime.evaluate", {"expression": js_test, "returnByValue": True, "awaitPromise": True})
print("Async submit test:", res)
