...
(function() {
    ...
    for (var el of els) {
        ...
        if (txt.includes('Edited') || txt.includes('Ran ') || txt.includes('Running') || txt.includes('Thought') || txt.includes('Thinking')) {
            if (txt.length < 200 && !items.includes(txt)) {
                items.push(txt);
            }
        }
    }
    return JSON.stringify(items.slice(-15));
})()
"""
