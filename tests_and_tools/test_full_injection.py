...
def test_inject(text):
    ...
    (function() {{
        ...
        // Insert text natively
        var textToInsert = {escaped_text};
        var ok = document.execCommand('insertText', false, textToInsert);
        ...
        if (sendBtn) {{
            sendBtn.click();
            return 'SUCCESS: text inserted and send button clicked!';
        }} else {{
            return 'TEXT_INSERTED_BUT_NO_SEND_BTN: ' + input.innerText.trim();
        }}
    }})()
    """
    res = cdp_send("Runtime.evaluate", {"expression": js, "returnByValue": True})
    return res

print(test_inject("Tes injeksi 100% sempurna dari CDP"))
