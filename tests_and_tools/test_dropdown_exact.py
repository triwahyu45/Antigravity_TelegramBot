...
def switch_to_exact_model(query):
    ...
    (async function() {{
        ...
        var q = {escaped_query}.toLowerCase();
        
        // Find matching item in dropdown list
        var items = Array.from(document.querySelectorAll('div, button, span, li, a'));
        var targetEl = items.find(function(el) {{
            if (el.offsetParent === null) return false;
            var txt = (el.innerText || el.textContent || '').trim().toLowerCase();
            return txt.includes(q) && el !== modelBtn && !modelBtn.contains(el);
        }});
        
        if (targetEl) {{
            var name = (targetEl.innerText || targetEl.textContent || '').trim();
            targetEl.click();
            await new Promise(r => setTimeout(r, 200));
            return 'OK:' + name.split('\\n')[0];
        }}
...
if __name__ == "__main__":
    q = sys.argv[1] if len(sys.argv) > 1 else "Sonnet"
    print(f"Testing switch to '{q}'...")
    res = switch_to_exact_model(q)
    print("Result:", res)
