...
(async function() {
    ...
    var all = Array.from(document.querySelectorAll('div, button, span, li, a'));
    var models = [];
    for (var el of all) {
        if (el.offsetParent === null) continue;
        var t = (el.innerText || el.textContent || '').trim();
        if ((t.includes('Gemini') || t.includes('Claude') || t.includes('GPT') || t.includes('Flash') || t.includes('Pro') || t.includes('Sonnet') || t.includes('Opus') || t.includes('Thinking')) && t.length < 50) {
            models.push(t);
        }
    }
    
    document.body.click();
    return JSON.stringify(Array.from(new Set(models)));
})()
"""
...
if res and 'result' in res and 'result' in res['result']:
    print("ALL_MODELS:", res['result']['result'].get('value'))
