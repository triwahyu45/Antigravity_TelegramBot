...
(function() {
    ...
    for (var el of els) {
        ...
        if (!txt || txt.length > 250 || seenText.has(txt)) continue;
        ...
        if (txt.startsWith('Edited ')) {
            isProgress = true;
            formatted = '✏️ *' + txt + '*';
        } else if (txt.startsWith('Ran ') || txt.startsWith('Running ')) {
            isProgress = true;
            formatted = '💻 *' + txt + '*';
        } else if (txt.startsWith('Thought ') || txt.startsWith('Thinking ')) {
            isProgress = true;
            formatted = '🧠 *' + txt + '*';
        } else if (txt.startsWith('Explored ')) {
            isProgress = true;
            formatted = '🔍 *' + txt + '*';
        } else if (txt.startsWith('Worked for ')) {
            isProgress = true;
            formatted = '⚙️ *' + txt + '*';
        }
        ...
        if (isProgress) {
            seenText.add(txt);
            formattedBadges.push(formatted);
        }
    }
    return JSON.stringify(formattedBadges);
})()
"""
