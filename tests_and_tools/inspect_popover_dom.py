...
(async function() {
    ...
    if (!modelBtn) return 'NO_MODEL_BTN';
    
    modelBtn.click();
    await new Promise(r => setTimeout(r, 300));
    
    // Find all div/button/li that contain ONLY the model title, e.g. text includes "Claude Sonnet" or "Gemini 3.1 Pro"
    var elements = Array.from(document.querySelectorAll('button, div[role="menuitem"], div.cursor-pointer, div.px-2, div.py-1, div.flex'));
    var result = [];
    for (var el of elements) {
        var txt = (el.innerText || el.textContent || '').trim();
        if (txt.length > 3 && txt.length < 80 && (txt.includes('Gemini') || txt.includes('Claude') || txt.includes('GPT') || txt.includes('Flash') || txt.includes('Pro') || txt.includes('Sonnet') || txt.includes('Opus'))) {
            result.push({
                tag: el.tagName,
                cls: el.className,
                text: txt
    ...
    document.body.click();
    return JSON.stringify(result.slice(-25));
})()
"""
...
if res and 'result' in res and 'result' in res['result']:
    print("POPOVER_ELEMENTS:", res['result']['result'].get('value'))
