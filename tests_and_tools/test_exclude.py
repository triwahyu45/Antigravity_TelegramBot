...
(function() {
    ...
    // Find conversation message blocks
    var messages = [];
    var userDivs = document.querySelectorAll('div.whitespace-pre-wrap.text-sm');
    for (var el of userDivs) {
        if (el.closest('[contenteditable="true"]')) continue;
        if (inputArea && inputArea.contains(el)) continue;
        messages.push({type: 'user', text: el.innerText.trim()});
    }
    
    return JSON.stringify({inputFound: !!inputEl, msgsCount: messages.length, sample: messages.slice(-3)});
})()
"""
print(cdp_eval(js_test))
