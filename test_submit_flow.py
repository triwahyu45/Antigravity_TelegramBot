...
(function() {
    ...
    // Insert text
    document.execCommand('insertText', false, 'Tes submit flow 1');
    
    // Check send button state immediately
    var btn = document.querySelector('button[aria-label="Send message"]') || document.querySelector('button[aria-label*="Send"]');
    
    var info = {
        btnFound: !!btn,
        btnDisabled: btn ? btn.disabled : null,
        btnClass: btn ? btn.className : '',
        btnAttributes: btn ? Array.from(btn.attributes).map(a => a.name + '=' + a.value) : []
    };
    return JSON.stringify(info);
})()
"""

print(cdp_send("Runtime.evaluate", {"expression": js_test, "returnByValue": True}))
