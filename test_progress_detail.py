...
(function() {
    ...
    var progressBlocks = [];
    var els = document.querySelectorAll('button[class*="text-left"], div[class*="text-left"]');
    for (var el of els) {
        if (inputArea && inputArea.contains(el)) continue;
        var txt = (el.innerText || '').trim();
        if (txt.length > 3 && (txt.includes('Edited') || txt.includes('Ran') || txt.includes('Running') || txt.includes('Explored') || txt.includes('Thought for') || txt.includes('Worked for'))) {
            progressBlocks.push(txt.replace(/\\n/g, ' '));
        }
    }
    return JSON.stringify(progressBlocks.slice(-10));
})()
"""
