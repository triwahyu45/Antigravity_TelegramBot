"""Debug: scrape current AI response from DOM and print it"""
import os, sys, json, socket, struct, base64, urllib.request, psutil, hashlib
from urllib.parse import urlparse
sys.stdout.reconfigure(encoding='utf-8')

def get_cdp():
    ag_pids = set()
    for p in psutil.process_iter(['pid','name']):
        try:
            if 'antigravity' in (p.info['name'] or '').lower(): ag_pids.add(p.info['pid'])
        except: pass
    for c in psutil.net_connections(kind='tcp'):
        try:
            if c.status=='LISTEN' and c.laddr.ip=='127.0.0.1' and c.pid in ag_pids:
                r=urllib.request.urlopen(f"http://127.0.0.1:{c.laddr.port}/json/version",timeout=1)
                if 'Browser' in json.loads(r.read()): return c.laddr.port
        except: pass
    return None

def cdp_eval(js):
    port = get_cdp()
    if not port: return None
    req = urllib.request.urlopen(f"http://127.0.0.1:{port}/json",timeout=3)
    ws_url = next((t['webSocketDebuggerUrl'] for t in json.loads(req.read()) if t.get('type')=='page'),None)
    if not ws_url: return None
    path = urlparse(ws_url).path
    s = socket.socket(); s.connect(('127.0.0.1',port)); s.settimeout(8)
    key = base64.b64encode(os.urandom(16)).decode()
    s.sendall(f"GET {path} HTTP/1.1\r\nHost: 127.0.0.1:{port}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: {key}\r\nSec-WebSocket-Version: 13\r\n\r\n".encode())
    resp=b''
    while b"\r\n\r\n" not in resp: resp+=s.recv(4096)
    p2=json.dumps({"id":1,"method":"Runtime.evaluate","params":{"expression":js,"returnByValue":True,"awaitPromise":True}}).encode()
    ln=len(p2); mk=os.urandom(4)
    h=bytes([0x81,0x80|(ln if ln<126 else 126)])
    if ln>=126: h+=struct.pack('>H',ln)
    h+=mk; s.sendall(h+bytes(b^mk[i%4] for i,b in enumerate(p2)))
    buf=b''
    for _ in range(40):
        chunk=s.recv(65536)
        if not chunk: break
        buf+=chunk
        while len(buf)>=2:
            l=buf[1]&0x7F; hlen=2
            if l==126:
                if len(buf)<4: break
                l=struct.unpack('>H',buf[2:4])[0]; hlen=4
            tot=hlen+l
            if len(buf)<tot: break
            fdata=buf[hlen:tot]; buf=buf[tot:]
            try:
                msg=json.loads(fdata.decode('utf-8'))
                if msg.get('id')==1: s.close(); return msg.get('result',{}).get('result',{}).get('value')
            except: pass
    s.close()
    return None

js = """
(function() {
    var isBusy = !!(document.querySelector('.animate-spin') || Array.from(document.querySelectorAll('button')).find(function(b){return b.innerText&&b.innerText.includes('Stop')}));
    var containers = document.querySelectorAll('div.leading-relaxed.select-text.text-sm, [class*="prose"]');
    var results = [];
    for (var c of containers) {
        var txt = (c.innerText||'').trim();
        if (txt.length > 10) results.push(txt.substring(0,200));
    }
    return JSON.stringify({isBusy: isBusy, count: results.length, samples: results.slice(-3)});
})()
"""

val = cdp_eval(js)
print("DOM_SCRAPE_DEBUG:", val)

# Also check seen file
SEEN_FILE = r"G:\Antigravity_Server\dom_mirror_final_seen.json"
if os.path.exists(SEEN_FILE):
    data = json.load(open(SEEN_FILE, encoding='utf-8'))
    print(f"SEEN HASHES COUNT: {len(data)}")
